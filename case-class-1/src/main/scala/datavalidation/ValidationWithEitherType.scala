...
object ValidationWithEitherType extends App :
  ...
  def validateEmailEither(string: String): Either[Error, Unit] = string match
    case null =>
  ...
  val validated: Either[Error, Unit] = validateEmailEither("hernan#email.com")
  ...
  object SSN:
    def fromString(string: String): Either[Throwable, SSN] = string match
      case null =>
        Left(Error("Social security is null"))
      case _ =>
        val split = string.split("-")
        if (split.size != 3)
  ...
  // employeeBadEmail: Validation error: java.lang.Error: Email 'hernan#email.com' is malformed
  ...
  // employeeBadSsn: Validation error: java.lang.Error: Three different sets of digits expected but 2 found
